...
def vggvox_resnet2d(input_dim=(257, 129, 1), mode='train', args=None, include_top=True):
    weight_decay = 1e-4
    ...
    if net == 'resnet34s':
        inputs_x, x = backbone.resnet_2D_v1(input_dim=input_dim, mode=mode, no_net='audio')
        # inputs_y, y = backbone.resnet_2D_v1(input_dim=input_dim, mode=mode, no_net=2)
    else:
        inputs_x, x = backbone.resnet_2D_v2(input_dim=input_dim, mode=mode, no_net='audio')
        # inputs_y, y = backbone.resnet_2D_v2(input_dim=input_dim, mode=mode, no_net=2)
    ...
    if not include_top:
        return Model(inputs=inputs_x, outputs=x)

    # combine the output of the two branches
    # x = keras.layers.concatenate([mx.output, my.output])
    ...
    model = keras.models.Model(inputs_x, y, name='vggvox_resnet2D_{}_{}'.format(loss, aggregation))
