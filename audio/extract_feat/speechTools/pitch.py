...
def getReaperPitches( signal=[], sampleRate=16000, wavFile=None):
	...
	os.system( "reaper -i " + wavFile + " -f " + tempPitchFile.name + " -a")
...
def pitches2semitones ( pitches, scale):
	...
	This function converts a pitch signal on a semitone sequence. Each pitch measure is replaced by the corresponding semitone. The semitone is chosen in the provided tonal scale. Each semitone is represented by its position in the tonal scale starting with (between 1 and scale.size included) for introducing spectral linearity and avoiding null values related troubles.
	In the context of speech, this operation keeps only the macro melodic component in the pitch signal..
