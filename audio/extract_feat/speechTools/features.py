...
#Energy and VAD intersection
	energies = st.collectEnergies ( signal )
	vadMask = st.getVadMask(signal, sampleRate)
	energies *= vadMask
	#for each energies windo compute the statistic features
	energyFeatures = []
...
def extractSnrFeatures ( signal, sampleRate=16000, windowSize=30,windowStep=10 ):
	...
	snrIndex = 0
	#get the minimum SNR coresponding to silence
	minSnr = np.min(snrs)
	#SNR and VAD intersection
	vadMask = st.getVadMask(signal, sampleRate)
	#read the VAD mask through windows that correspond to the default SNR sampling
	vadMaskReader = st.getSignalReader( vadMask, 100, 1, 0.5)
	for window in vadMaskReader:
	#if the VAD window is not active: rectify the corresponding SNR value setting it to the minimum value
		if np.mean(window) <=  0.5: snrs[snrIndex] = minSnr
		snrIndex +=1
	#for each SNR window compute the statistic features
	snrFeatures = []
	snrReader = st.getSignalReader( snrs, 2, windowSize, windowStep)
	for window in snrReader:
	#compute the statistic features excluding the previously found minimum SNR value that corresponds to silence
...
#float casting for librosa compatibility
	floatSignal = np.float32(signal)
	#get spectral centroid features
	...
	#spectral centroid and VAD intersection
	vadMask = st.getVadMask( signal, sampleRate)
	spectralCentroids, vadMask = st.equalizeShapes( spectralCentroids, vadMask)
	spectralCentroids *= vadMask
	#convert spectral centroids to semitone for spectral linearity
	...
	#for each spectral centroids window compute the statistic features
	semitoneFeatures = []
	semitoneReader = st.getSignalReader( semitones, 100, windowSize, windowStep)
	for window in semitoneReader:
		localFeatures = computeLocalFeatures( window )
	...
	#exactly same aproach for spectral flatness
